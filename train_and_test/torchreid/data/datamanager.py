...
class MyDataset(Dataset):
    ...
    def __init__(self, root="project/data", **kwargs):
        ...
        #         z.extractall(path=root)  
        self.train_dir = osp.join(root, "MyDataset_V2/bounding_box_train_remove_1_and_2_v2")  # change here to adjust the train set
        # self.list_train_path = osp.join(root, 'train_list.txt')  # useless, we have already rename the image except the raw test set
        self.val_query_dir = osp.join(root, "MyDataset_V2/query_v2")  # change here to adjust the val set query
        self.val_gallery_dir = osp.join(root, "MyDataset_V2/bounding_box_test_distractor_v2")  # change here to adjust the val set gallery
        self.query_dir = osp.join(root, "chu_test_b/chu_test_b/query_b/query_b")  # true query
        self.gallery_dir = osp.join(root, "chu_test_b/chu_test_b/gallery_b/gallery_b")  # true gallery, no pid label
        # 如果是对提取的特征聚类打伪标签，则将该行的上一行换成test_b，而test_b是query_b和gallery_b的合并结果
...
class MyDataManager():
    def __init__(self, root="project/data", height=256, width=128,
...
class MyDataset_FineTune(Dataset):
    def __init__(self, root="project/ywchong/ywchong/CODE/zc/AIC/data", **kwargs):
        self.root = osp.abspath(osp.expanduser(root))
        self.train_dir = osp.join(root, "MyDataset_V2/fine_tune_dataset/test_a")  
        ...
        val_query = self.process_dir(self.val_query_dir)
        val_gallery = self.process_dir(self.val_gallery_dir)
        query = self.process_dir(self.query_dir, no_pid=True)   
        gallery = self.process_dir(self.gallery_dir, no_pid=True)

        super(MyDataset_FineTune, self).__init__(train, val_query, val_gallery, query, gallery, **kwargs)
...
class MyDataManager_FineTune():
    def __init__(self, root="project/ywchong/ywchong/CODE/zc/AIC/data", height=256, width=128,
        ...
        self.width = width
        self.use_gpu = (torch.cuda.is_available() and use_gpu)
        super(MyDataManager_FineTune, self).__init__()
        ...
        print('=> Loading train dataset')
        trainset = MyDataset_FineTune(root, transform=self.transform_tr, mode='train')
        train_sampler = torchreid.data.sampler.build_train_sampler(
            trainset.train, train_sampler,
            batch_size=batch_size_train,
            num_instances=num_instances,
        ...
        self.valloader = {'val_query': None, 'val_gallery': None}
        self.valdataset = {'val_query': None, 'val_gallery': None}
        val_queryset = MyDataset_FineTune(root, transform=self.transform_te, mode='val_query')
        self.valloader['val_query'] = torch.utils.data.DataLoader(
            val_queryset,
            batch_size=batch_size_test,
            shuffle=False,
            ...
            pin_memory=self.use_gpu,
            drop_last=False)
        val_galleryset = MyDataset_FineTune(root, transform=self.transform_te, mode='val_gallery', verbose=False)
        self.valloader['val_gallery'] = torch.utils.data.DataLoader(
            val_galleryset,
            batch_size=batch_size_test,
            shuffle=False,
        ...
        self.testloader = {'query': None, 'gallery': None}
        self.testdataset = {'query': None, 'gallery': None}
        queryset = MyDataset_FineTune(root, transform=self.transform_te, mode='query')
        self.testloader['query'] = torch.utils.data.DataLoader(
            queryset,
            batch_size=batch_size_test, 
            shuffle=False,
            ...
            pin_memory=self.use_gpu,
            drop_last=False)
        galleryset = MyDataset_FineTune(root, transform=self.transform_te, mode='gallery', verbose=False)
        self.testloader['gallery'] = torch.utils.data.DataLoader(
            galleryset,
            batch_size=batch_size_test, 
            shuffle=False,
...
class MyDataset_FineTune_b(Dataset):
    def __init__(self, root="project/ywchong/ywchong/CODE/zc/AIC/data", **kwargs):
        self.root = osp.abspath(osp.expanduser(root))
        self.train_dir = osp.join(root, "MyDataset_V2/fine_tune_dataset/test_b")  
        self.val_query_dir = osp.join(root, "MyDataset_V2/query_v2") 
        self.val_gallery_dir = osp.join(root, "MyDataset_V2/bounding_box_test_distractor_v2") 
        self.query_dir = osp.join(root, "chu_test_b/chu_test_b/query_b/query_b") 
        self.gallery_dir = osp.join(root, "chu_test_b/chu_test_b/gallery_b/gallery_b")  
        ...
        super(MyDataset_FineTune_b, self).__init__(train, val_query, val_gallery, query, gallery, **kwargs)
...
class MyDataManager_FineTune_b():
    def __init__(self, root="project/ywchong/ywchong/CODE/zc/AIC/data", height=256, width=128,
                 transforms='random_flip', norm_mean=None, norm_std=None, use_gpu=True, batch_size_train=32,
                 batch_size_test=32, workers=4, num_instances=4, train_sampler='', loss='softmax',):
        ...
        self.width = width
        self.use_gpu = (torch.cuda.is_available() and use_gpu)
        super(MyDataManager_FineTune_b, self).__init__()
        ...
        print('=> Loading train dataset')
        trainset = MyDataset_FineTune_b(root, transform=self.transform_tr, mode='train')
        train_sampler = torchreid.data.sampler.build_train_sampler(
            trainset.train, train_sampler,
            batch_size=batch_size_train,
            num_instances=num_instances,
        ...
        self.valloader = {'val_query': None, 'val_gallery': None}
        self.valdataset = {'val_query': None, 'val_gallery': None}
        val_queryset = MyDataset_FineTune_b(root, transform=self.transform_te, mode='val_query')
        self.valloader['val_query'] = torch.utils.data.DataLoader(
            val_queryset,
            batch_size=batch_size_test,
            shuffle=False,
            ...
            pin_memory=self.use_gpu,
            drop_last=False)
        val_galleryset = MyDataset_FineTune_b(root, transform=self.transform_te, mode='val_gallery', verbose=False)
        self.valloader['val_gallery'] = torch.utils.data.DataLoader(
            val_galleryset,
            batch_size=batch_size_test,
            shuffle=False,
        ...
        self.testloader = {'query': None, 'gallery': None}
        self.testdataset = {'query': None, 'gallery': None}
        queryset = MyDataset_FineTune_b(root, transform=self.transform_te, mode='query')
        self.testloader['query'] = torch.utils.data.DataLoader(
            queryset,
            batch_size=batch_size_test, 
            shuffle=False,
            ...
            pin_memory=self.use_gpu,
            drop_last=False)
        galleryset = MyDataset_FineTune_b(root, transform=self.transform_te, mode='gallery', verbose=False)
        self.testloader['gallery'] = torch.utils.data.DataLoader(
            galleryset,
            batch_size=batch_size_test, 
            shuffle=False,
